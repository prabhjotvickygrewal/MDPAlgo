...
public class Simulator {
    ...
    public SensorData getSensorData(Robot r){
        ...
        data.left_l=getObstacleAt(pos.nAdd(leftVector).nAdd(downVector), ori.getLeft(), MapLayer.Sensor_LongRange);
        data.left_m=getObstacleAt(pos.nAdd(leftVector), ori.getLeft(), MapLayer.Sensor_LongRange);
        data.left_r=getObstacleAt(pos.nAdd(leftVector).nAdd(ori.toVector()), ori.getLeft(), MapLayer.Sensor_LongRange);
        data.right_l=getObstacleAt(pos.nAdd(rightVector).nAdd(ori.toVector()), ori.getRight(), MapLayer.Sensor_ShortRange);
        data.right_m=getObstacleAt(pos.nAdd(rightVector), ori.getRight(), MapLayer.Sensor_ShortRange);
        data.right_r=getObstacleAt(pos.nAdd(rightVector).nAdd(downVector), ori.getRight(), MapLayer.Sensor_ShortRange);
